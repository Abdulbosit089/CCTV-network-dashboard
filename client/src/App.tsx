...
const App: React.FC = () => {
  ...
  const handleChangePassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setAlertDialog({ isOpen: true, message: 'Password changed successfully' });
      } else {
        const json = await res.json();
        setAlertDialog({ isOpen: true, message: json.error || 'Failed to change password' });
      }
    } catch (err) {
      setAlertDialog({ isOpen: true, message: 'Error connecting to server' });
    }
  };

  const handleUpdateCapacity = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const capacity = Number(formData.get('capacity'));

    try {
      const res = await authFetch(`${API_BASE}/settings/capacity`, {
        method: 'POST',
      ...
      if (res.ok) {
        ...
        fetchDashboardData();
        setAlertDialog({ isOpen: true, message: 'Network capacity updated successfully' });
      } else {
        const json = await res.json();
        setAlertDialog({ isOpen: true, message: json.error || 'Failed to update capacity' });
      }
    } catch (err) {
      setAlertDialog({ isOpen: true, message: 'Error connecting to server' });
    }
    ...
    try {
      ...
      if (err.name !== 'AuthError') {
        if (err.message === 'Failed to fetch') {
           console.warn('Network: Waiting for server to reconnect...');
        } else {
  ...
  const resetDatabase = () => {
    setConfirmDialog({
      isOpen: true,
      message: 'Are you sure you want to reset the simulation database?',
      onConfirm: async () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100">
      ...
      <div className="fixed left-0 top-0 bottom-0 w-16 bg-slate-900 flex flex-col items-center py-8 gap-8 border-r border-slate-800 z-50 hidden md:flex">
        ...
        {showSettings && (
          <div className="absolute left-20 bottom-8 bg-slate-900 border border-slate-800 rounded-xl p-2 shadow-2xl w-48 z-[60] animate-in slide-in-from-left-2 duration-200">
            <button 
              onClick={() => { setShowCapacityModal(true); setShowSettings(false); }}
      ...
      <main className="md:ml-16 p-4 md:p-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <StatCard 
            ...
            subValue={`${totalBitrate.toFixed(1)} / ${capacity} Mbps`}
            trend={Number(loadPercentage) > 80 ? 'up' : 'stable'}
            color={Number(loadPercentage) > 80 ? 'red' : 'blue'}
          />
          <StatCard 
            label="Network Latency" 
            value={`${Number(currentMetric?.latency || 0).toFixed(2)}ms`} 
            subValue="Avg Round Trip"
            trend={currentMetric.latency > 100 ? 'up' : 'stable'}
            color={currentMetric.latency > 100 ? 'orange' : 'emerald'}
          />
          <StatCard 
            label="Packet Loss" 
            value={`${Number(currentMetric?.packet_loss || 0).toFixed(2)}%`} 
            subValue="Transmission Error"
            trend={currentMetric.packet_loss > 1 ? 'up' : 'stable'}
            color={currentMetric.packet_loss > 1 ? 'red' : 'emerald'}
          />
          <StatCard 
            label="Active Nodes" 
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="text-[9px] font-bold px-2 py-1 rounded-md bg-red-50 text-red-600 hover:bg-red-100 transition-all border border-red-100"
                          title="Remove Camera"
                        >
                          <Trash2 size={10} />
                        </button>
      ...
      {showCapacityModal && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl p-8 max-w-md w-full shadow-2xl border border-slate-200">
            <h2 className="text-xl font-bold text-slate-900 mb-6">Network Speed (Capacity)</h2>
            <form onSubmit={handleUpdateCapacity} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-slate-400 uppercase ml-1">Maximum Throughput (Mbps)</label>
                <input name="capacity" type="number" step="1" defaultValue={data?.capacity || 100} className="w-full border border-slate-200 p-2.5 rounded-lg bg-slate-50 text-sm focus:outline-none focus:border-blue-500" required />
                <p className="text-xs text-slate-500 mt-2 px-1">Set the actual bandwidth of your internet plan to see your camera impact accurately.</p>
              </div>
              <div className="flex gap-3 pt-6">
                <button type="button" onClick={() => setShowCapacityModal(false)} className="flex-1 border border-slate-200 py-3 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
                <button type="submit" className="flex-1 bg-blue-600 text-white py-3 rounded-lg text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-500 transition-all">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex gap-3">
              <button onClick={() => setConfirmDialog(null)} className="flex-1 border border-slate-200 py-2.5 rounded-lg text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">Cancel</button>
              <button onClick={confirmDialog.onConfirm} className="flex-1 bg-red-600 text-white py-2.5 rounded-lg text-sm font-bold shadow-lg shadow-red-200 hover:bg-red-500 transition-all">Confirm</button>
            </div>
          </div>
        </div>
      )}
      ...
      {alertDialog?.isOpen && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center z-[200] p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl border border-slate-200 animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-slate-600 mb-6">{alertDialog.message}</p>
            <button onClick={() => setAlertDialog(null)} className="w-full bg-blue-600 text-white py-2.5 rounded-lg text-sm font-bold shadow-lg shadow-blue-200 hover:bg-blue-500 transition-all">OK</button>
          </div>
        </div>
      )}
    </div>
