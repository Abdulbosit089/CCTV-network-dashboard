...
let globalNetworkCapacity = 100; // in Mbps
const memoryDb = {
  users: [] as any[],
  cameras: [
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      alerts: alerts.rows,
      logs: logs.rows,
      capacity: globalNetworkCapacity
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.delete('/api/cameras/:id', authenticateToken, async (req, res) => {
  try {
    if (useInMemory) {
      const index = memoryDb.cameras.findIndex(c => c.id === parseInt(req.params.id));
      if (index !== -1) {
        const camName = memoryDb.cameras[index].name;
        memoryDb.cameras.splice(index, 1);
        addLog(`CAMERA_REMOVED: ${camName}`);
        return res.json({ success: true });
      }
    } else {
      const result = await pool.query('DELETE FROM cameras WHERE id = $1 RETURNING *', [req.params.id]);
      const cam = result.rows[0];
      if (cam) {
        addLog(`CAMERA_REMOVED: ${cam.name}`);
        return res.json({ success: true });
      }
    }
    res.status(404).json({ error: 'Camera not found' });
...
app.post('/api/settings/capacity', authenticateToken, async (req, res) => {
  const { capacity } = req.body;
  try {
    if (!capacity || isNaN(capacity) || capacity <= 0) {
      return res.status(400).json({ error: 'Invalid capacity value' });
    }
    globalNetworkCapacity = Number(capacity);
    addLog(`NETWORK_CAPACITY_UPDATED: ${globalNetworkCapacity} Mbps`);
    res.json({ success: true, capacity: globalNetworkCapacity });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
const runSimulation = async () => {
  try {
    ...
    const capacity = globalNetworkCapacity;
